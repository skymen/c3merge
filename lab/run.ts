...
async function runOne(editor: C3Editor, release: string, c: Corruption | null, controlSaved: string | null): Promise<RowResult & { savedDir: string | null }> {
  ...
  const r: RowResult & { savedDir: string | null } = {
    row, title: c?.title ?? "untouched base project", release, editor: "error", dialogs: [], survivedIn: [], dropped: [], preview: "n/a", cause: "", detail: "", savedDir: null,
  };
  ...
  try {
    ...
    r.editor = rep.dialogs.length ? "loads with notice"
      ...
      : c && r.survivedIn.length === 0 ? "repairs silently" : "loads, keeps it";
...
try {
  for (const branchOrRelease of args.releases!.split(",")) {
    ...
    const rowResults = await pool(rows, tabs, async (c) => {
      const r = await runOne(editor, rel.name, c, control.savedDir).catch((e) => ({ row: c.row, title: c.title, release: rel.name, editor: "error" as const, dialogs: [], survivedIn: [], dropped: [], preview: "n/a", cause: "", detail: (e as Error).message.split("\n")[0], savedDir: null }));
      console.log(`   ${r.row.padEnd(4)} ${r.editor.padEnd(18)} preview: ${r.preview.slice(0, 50).padEnd(50)} ${r.survivedIn.length ? `kept in ${r.survivedIn.join(", ")}` : ""}`);
      return r;
    });
...
async function writeMatrix(results: RowResult[]) {
  ...
  const cell = (r?: RowResult) => {
    ...
    else if (r.survivedIn.length && r.row !== "control") bits.push(`kept in ${r.survivedIn.map((f) => `\`${f}\``).join(", ")}`);
    return bits.join("<br>");
  };
  const lines = [
    ...
    "",
    "- **loads, keeps it**: opens with no dialog, and \"Save as project folder\" (which writes every file from memory) still contains the corruption.",
    "- **loads, drops content**: opens with no dialog, but Save as no longer writes some of the project (listed): silent data loss.",
    "- **repairs silently**: opens with no dialog, and Save as writes exactly what it writes for the untouched project.",
