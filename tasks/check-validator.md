...
- instance `uid` duplicated within a layout or across layouts, rows 2 and 3. C3 does *not*
  regenerate them; effects on UID picking are unknown.
- event `sid` duplicated or missing, rows 4 and 5. Not regenerated either.
...
global variable (32), duplicate effect name (34). Row 21 (old `savedWithRelease`) needs a
real project from an old release: faking the number makes C3 look for the lowercase file
names old releases used.
